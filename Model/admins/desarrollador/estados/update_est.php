<?php
    ...
    $sql = $con -> prepare ("SELECT * FROM estados WHERE estados.id_estado = '".$_GET['id_estado']."'");
    $sql -> execute();
    $usua =$sql -> fetch();
?>
...
if(isset($_POST["update"]))
 {
    ...
    $insertSQL = $con -> prepare("UPDATE estados SET estado = '$estado' WHERE id_estado = '".$_GET['id_estado']."'");      
    $insertSQL->execute();
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $("#estado").on("input", function() {
                validateField(/^([a-zA-ZáéíóúÁÉÍÓÚñÑ\s]){4,20}$/, this, "Ingrese un nombre válido (solo letras)");
            });
        });

        function validateForm() {
            const isEstadoValid = validateField(/^([a-zA-ZáéíóúÁÉÍÓÚñÑ\s]){4,20}$/, document.getElementById("estado"), "Ingrese un nombre válido (solo letras)");
...
<body>
    ...
    <img src="../../../../assets/img/log.farma.png">
        ...
        <form method="POST" name="formreg" autocomplete="off">
            <div class="campos">
                <input type="text" name="estado" id="estado" value="<?php echo $usua['estado']?>">
            </div>
