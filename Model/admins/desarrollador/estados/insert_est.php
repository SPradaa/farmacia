...
if ((isset($_POST["MM_insert"]))&&($_POST["MM_insert"]=="formreg"))
{
    ...
    $sql= $con -> prepare ("SELECT * FROM estados WHERE estado='$estado'");
    $sql -> execute();
    $fila = $sql -> fetchAll(PDO::FETCH_ASSOC);

    if ($fila){
    echo '<script>alert ("EL ESTADO YA EXISTE //CAMBIELO//");</script>';
    echo '<script>window.location="insert_est.php"</script>';
    }
    else
    ...
    $insertSQL -> execute();
    echo '<script> alert("Estado registrado exitosamente");</script>';
...
<head>
    ...
    <script>
        ...
        function validateForm() {
            const isEstadooValid = validateField(/^([a-zA-ZáéíóúÁÉÍÓÚñÑ\s]){4,20}$/, document.getElementById("estado"), "Ingrese un nombre válido (solo letras)");
