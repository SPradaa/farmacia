...
if ((isset($_POST["MM_insert"]))&&($_POST["MM_insert"]=="formreg"))
{
    ...
    $sql= $con -> prepare ("SELECT * FROM rh WHERE rh='$rh'");
    $sql -> execute();
    $fila = $sql -> fetchAll(PDO::FETCH_ASSOC);

    if ($fila){
    echo '<script>alert ("TIPO DE SANGRE YA EXISTE //CAMBIELO//");</script>';
    echo '<script>window.location="insert_rh.php"</script>';
    }
    else
    ...
    {
    $insertSQL = $con->prepare("INSERT INTO rh(rh) VALUES('$rh')");
    $insertSQL -> execute();
    echo '<script> alert("Tipo de Sangre registrado exitosamente");</script>';
...
<head>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $("#rh").on("input", function() {
                validateField(/^([a-zA-Z-+\s]){2,3}$/, this, "Ingrese un RH válido (Ejemplo O+)");
            });
        });

        function validateForm() {
            const isRhValid = validateField(/^([a-zA-Z-+\s]){2,3}$/, document.getElementById("rh"), "Ingrese un RH válido (Ejemplo O+)");
    ...
    <img src="../../../../assets/img/log.farma.png">
        ...
        <form action="" method="post">
            <div class="form-group">
                <input type="text" name="rh" id="rh" placeholder="Rh" class="input__text">
            </div>
