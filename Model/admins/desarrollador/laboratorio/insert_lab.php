...
if ((isset($_POST["MM_insert"]))&&($_POST["MM_insert"]=="formreg"))
{
    ...
    $sql= $con -> prepare ("SELECT * FROM laboratorio WHERE laboratorio='$laboratorio'");
    $sql -> execute();
    $fila = $sql -> fetchAll(PDO::FETCH_ASSOC);

    if ($fila){
    echo '<script>alert ("EL LABORATORIO YA EXISTE //CAMBIELO//");</script>';
    echo '<script>window.location="insert_lab.php"</script>';
    }
    else
    ...
    {
    $insertSQL = $con->prepare("INSERT INTO laboratorio(laboratorio) VALUES('$laboratorio')");
    $insertSQL -> execute();
    echo '<script> alert("Laboratorio registrado exitosamente");</script>';
...
<head>
    ...
    <script>
        ...
        function validateForm() {
            const isLaboratorioValid = validateField(/^([a-zA-ZáéíóúÁÉÍÓÚñÑ\s]){5,30}$/, document.getElementById("laboratorio"), "Ingrese un nombre válido (solo letras)");

            return islaboratorioValid;
        }
    </script>
    <style>
   * {
...
<body>
    ...
    <img src="../../../../assets/img/log.farma.png">
        ...
        <form method="POST" name="formreg" autocomplete="off">
            <div class="campos">
                <input type="text" name="laboratorio" id="laboratorio" placeholder="Ingrese el nombre del Laboratorio">
            </div>
