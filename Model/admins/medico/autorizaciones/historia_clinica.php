...
// Realizar la consulta para obtener la cita del usuario con el documento dado
$sql_cita = "SELECT * FROM citas WHERE documento = :documento";
$stmt_cita = $con->prepare($sql_cita);
$stmt_cita->bindParam(':documento', $documento);
$stmt_cita->execute();
$cita = $stmt_cita->fetch(PDO::FETCH_ASSOC);

// Verificar si se encontró la cita
if ($cita) {
    // Verificar si el estado es diferente de 11
    if ($cita['id_estado'] != 11) {
        // Actualizar el estado a 11
        $sql_update = "UPDATE citas SET id_estado = 11 WHERE documento = :documento";
        $stmt_update = $con->prepare($sql_update);
        $stmt_update->bindParam(':documento', $documento);
        $stmt_update->execute();
        echo "El estado de la cita se actualizó a 11.";
    } else {
        echo "El estado de la cita ya es 11.";
    }
} else {
    echo "No se encontró ninguna cita para el documento dado.";
}
?>
